...
def _finite_result(value: float) -> float:
    if not math.isfinite(value):
        raise EvaluationNonFiniteError("solver evaluator produced a non-finite value")
...
def _apply_unary(op: str, operand: float) -> float:
    ...
    if op == "negative":
        return _finite_result(-operand)
    raise EvaluationDomainError("solver evaluator received an unknown unary operator")
...
def _apply_binary(op: str, left: float, right: float) -> float:
    try:
        ...
        raise EvaluationNonFiniteError("solver evaluator overflowed") from error
...
def _apply_call(name: str, argument: float) -> float:
    try:
        if name == "abs":
            ...
            raise EvaluationDomainError("solver evaluator received an unknown function")
    except ValueError as error:
        raise EvaluationDomainError("solver evaluator function domain error") from error
    except OverflowError as error:
        raise EvaluationNonFiniteError("solver evaluator function overflow") from error
    return _finite_result(result)
...
def _evaluate_node(
    ...
    if isinstance(ast, UnaryNode):
        return _apply_unary(ast.op, _evaluate_node(ast.operand, x, budget, depth + 1))
    if isinstance(ast, BinaryNode):
        if ast.op == "power" and (
            not isinstance(ast.right, NumberNode)
            or not -1024.0 <= ast.right.value <= 1024.0
        ):
            raise EvaluationDomainError("solver evaluator rejected an unsafe exponent")
    ...
    raise EvaluationDomainError("solver evaluator received an unknown AST node")
...
class SolverEvaluator:
    ...
    def evaluate(self, ast: ExpressionAst, x: float, budget: EvaluationBudget) -> float:
        ...
        if (
            ...
            raise EvaluationNonFiniteError("solver x must be finite binary64")
